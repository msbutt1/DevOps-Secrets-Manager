...
- [x] **Forgot password.** Emailed reset token, then a reset page. Commit: `feat: password reset by email`
...
- [x] **Version history.** Keep previous encrypted values (a `secret_versions` table) so a bad update can be rolled back. Record the version number in the audit log. Commit: `feat: secret version history and rollback`
- [x] **Bulk import and export.**
...
- [x] **Encryption tests.** Tests exist for `envelope.go` only. Add tests for:
...
- [x] **Token file fallback.** (Found while documenting token storage.) When the OS keyring is unavailable the CLI writes `tokens.json` with default permissions, base64-encoded but described as an "encrypted file". Write it with mode 0600 and describe it honestly, still reading old files. Commit: `fix(cli): write token file fallback privately`
...
- [x] **Threat model.** Write `SECURITY.md`: what the encryption protects against, what it doesn't (a compromised API host), how to report issues, and how to rotate keys. Commit: `docs: security model and reporting`
...
- [x] **Confirmations.** Deleting a vault, environment or secret, or removing a member, requires typing the name. Commit: `feat(web): typed confirmation for destructive actions`
  ...
  - Also found: the vault header's Edit button had no handler at all; it now opens the vault form (`fix(web): make the vault Edit button work`).
- [x] **Keyboard and accessibility.** Focus order, visible focus, labelled icon buttons, dialogs that trap and restore focus, checked with axe. Commit: `fix(web): accessibility pass`
